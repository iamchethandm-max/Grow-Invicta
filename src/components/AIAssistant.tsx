  ...
  const executeAction = (action: { type: string; payload: any }) => {
    ...
    if (type === 'CREATE_INVOICE') {
      ...
      const leadObj: Lead = {
        ...
        phone: payload.phone || '',
        email: payload.email || '',
        source: payload.source || 'AI Sourced',
      ...
      const projectObj: Project = {
        id: `p_ai_${Date.now()}`,
        title: payload.title || 'AI Sourced Development Node',
        clientName: payload.clientName || 'General Client',
        value: Number(payload.value) || 150000,
        status: payload.status || 'Planning',
        startDate: new Date().toISOString().split('T')[0],
        endDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        notes: payload.notes || 'Sourced and initiated by AI core.',
        tasksCount: 0,
      ...
      desc = `Successfully launched project **${projectObj.title}** for client **${projectObj.clientName}** valued at **₹${projectObj.value.toLocaleString('en-IN')}**.`;
      ...
      setAuditLogs(prev => [
        {
          ...
          details: `Launched project node "${projectObj.title}" for client ${projectObj.clientName}.`
        },
        ...prev
      ]);
  ...
  const handleSend = async (messageText = input) => {
    ...
    try {
      ...
      const appStateContext = {
        ...
        projects: projects.map(p => ({ id: p.id, title: p.title, clientName: p.clientName, value: p.value, status: p.status })),
